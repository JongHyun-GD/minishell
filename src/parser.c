...
static void	organize_quote(t_node *node)
{
	...
	while (data[index])
	{
		...
		printf("&data[%d]: %s\n", index, &data[index]);
		printf("len: %d\n", len);
		printf("ft_substr: %s\n", ft_substr(&data[index], 0, len));
		if (index == 0)
		{
			free(node->data);
			node->data = ft_substr(&data[index], 0, len);
		}
		else 
			ft_nodeadd_back(node, ft_create_node(NTYPE_STRING, ft_substr(&data[index], 0, len)));
		index += len;
		printf("\n");
	}
	free(data);
	printf("# organize_quote OUT\n");
}
